...
#if defined(CONFIG_MPL115) || defined(CONFIG_MPL115_MODULE)
	{
		I2C_BOARD_INFO("mpl115a", 0x60),
	},
#endif
#if defined(CONFIG_INPUT_MMA7455L_I2C) || defined(CONFIG_INPUT_MMA7455L_I2C_MODULE)
	#define		MMA7455_INT1			130
	{
		I2C_BOARD_INFO("mma7455l", 0x1D),
		.irq = OMAP_GPIO_IRQ(MMA7455_INT1),
	},
#endif
};
...
static void go_LowPower() {
    u32 val;
    u32 mask = (0b100 << 16) | 0b100;
    int i;

    val = omap_readl(0x480020D4);
    omap_writel(val | mask, 0x480020D4);
    val = omap_readl(0x480020D8);
    omap_writel(val | mask, 0x480020D8);
    val = omap_readl(0x480020DC);
    omap_writel(val | mask, 0x480020DC);
    val = omap_readl(0x480020E0);
    omap_writel(val | mask, 0x480020E0);
    val = omap_readl(0x480020E4);
    omap_writel(val | mask, 0x480020E4);
    val = omap_readl(0x480020E8);
    omap_writel(val | mask, 0x480020E8);
    val = omap_readl(0x480020EC);
    omap_writel(val | mask, 0x480020EC);
    val = omap_readl(0x480020F0);
    omap_writel(val | mask, 0x480020F0);
    val = omap_readl(0x480020F4);
    omap_writel(val | mask, 0x480020F4);
    val = omap_readl(0x480020F8);
    omap_writel(val | mask, 0x480020F8);
    val = omap_readl(0x480020FC);
    omap_writel(val | mask, 0x480020FC);
...
static struct platform_device zkpk_bl =
{
		...
		unsigned gpio, unsigned ngpio)
{
	...
	gpio_request(gpio + 6, "LCD Backlight");
	gpio_direction_output(gpio + 6, 1);
	gpio_export(gpio + 6, 1);
...
//	omap_register_i2c_bus(3, 100, beagle_i2c_eeprom, ARRAY_SIZE(beagle_i2c_eeprom));
    ...
    blueshark_max7359data.debounce_reg_val = 0x1F;
  } else if (strcmp(expansionboard_name, "zkpk") == 0) {
